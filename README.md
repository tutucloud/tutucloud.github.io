...
构建压缩产物并部署到 GitHub Pages（tutucloud.github.io）：

```bash
npm install
npm run deploy   # = npm run build + 强制推送 dist/ 到线上仓库
```
...
- UUID v4 批量生成
- 文本工具：去重 / 排序 / 去空行 / 字数统计
- 颜色转换：HEX ⇄ RGB ⇄ HSL，带预览色块
- 文本对比（行级 Diff，LCS 算法）
- 正则表达式实时测试（高亮匹配）
- 二维码生成（内置零依赖编码器，纠错级别 M，最长约 213 字节）
- 代码压缩 / 美化：CSS 压缩与美化（字符串感知）、JS 美化（按括号重缩进）
